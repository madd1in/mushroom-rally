"""R44 Update-Video, Schritt 2 (nach art/r39/edit_video_blender.py): Schnitt und Ton im Blender-Videoschnitt
(VSE), Export als MP4. Bilder aus art/r44/capture_frames.mjs (.scratch/r44-frames). Toene: Renn-Musik plus die
Chiptune-Effekte des Spiels (assets/audio/sfx/chip), gelegt auf die protokollierten Ereignisse.
Aufruf (Kommandozeile): blender -b --factory-startup --python art/r44/edit_video_blender.py
Erzeugt media/r44_strecken.mp4 (1080x1920) und media/r44_strecken_small.mp4 (810x1440, < 10 MB).
"""
import bpy, json, os, pathlib

ROOT = pathlib.Path(r'C:/Users/User/Documents/Playground/mushroom-rally')
FR = ROOT / '.scratch' / 'r44-frames'
AU = ROOT / 'assets' / 'audio'
R44 = ROOT / 'art' / 'r44'
CH = AU / 'sfx' / 'chip'
OUT = ROOT / 'media' / 'r44_strecken.mp4'
FPS, END_SEC = 30, 4.5
ev = json.loads((FR / 'events.json').read_text(encoding='utf-8'))
N = ev['frames']

sc = bpy.data.scenes.get('R44_Video') or bpy.data.scenes.new('R44_Video')
...
MAP = [('MINI-TURBO', [(CH / 'mt1.wav', 0, .5)]), ('SUPER-TURBO', [(CH / 'mt2.wav', 0, .5)]), ('ULTRA-TURBO', [(CH / 'mt3.wav', 0, .55)]),
...
render(ROOT / 'media' / 'r44_strecken_small.mp4', 75, kbps)
report.update({'seconds': round(secs, 1), 'small_kbps': kbps,
               'big_bytes': OUT.stat().st_size, 'small_bytes': (ROOT / 'media' / 'r44_strecken_small.mp4').stat().st_size})
